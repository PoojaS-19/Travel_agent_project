...
export default function ProfilePage() {
  ...
  // Active Tab: "saved" or "finished"
  const [activeTab, setActiveTab] = useState("saved");

  // Filtered trips based on active tab
  const filteredTrips = useMemo(() => {
    return trips.filter((trip) => {
      const isFinished = trip.is_finished || false;
  ...
  // When changing tabs, automatically select the first trip of that tab if none is selected
  useEffect(() => {
    if (filteredTrips.length > 0) {
      // If current selected trip is not in the filtered list, select the first one
      const stillVisible = filteredTrips.some((t) => t.id === selectedTrip?.id);
      if (!stillVisible) {
    ...
    try {
      ...
      setSelectedTrip((current) => {
        ...
        if (tripIdFromUrl) {
          ...
          if (found) {
            // Set correct tab based on trip status
            setActiveTab(found.is_finished ? "finished" : "saved");
            return found;
          }
        }
        if (current) {
          const found = loadedTrips.find((trip) => trip.id === current.id);
          if (found) return found;
        }
        
        // Default: find the first active trip
        const firstActive = loadedTrips.find((t) => !t.is_finished);
        if (firstActive) {
          setActiveTab("saved");
          return firstActive;
        }
        const firstFinished = loadedTrips.find((t) => t.is_finished);
        if (firstFinished) {
  ...
  const toggleFinishTrip = async (trip) => {
    setSaving(true);
    setError("");
    setMessage("");
    try {
      const nextStatus = !trip.is_finished;
      const response = await API.put(`/itineraries/${trip.id}`, {
        is_finished: nextStatus
      });
      const updatedTrip = response.data;
      ...
      setSelectedTrip(updatedTrip);
      setMessage(nextStatus ? "Trip marked as finished!" : "Trip moved back to saved itineraries.");
    } catch (err) {
      console.error("Toggle finish trip error:", err);
      setError(err.response?.data?.detail || "Could not update trip status.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="saved-trips-page">
      ...
      {user && (
        <div style={{
          ...
          <div>
            <h2 style={{ margin: "0 0 4px 0", fontSize: "24px", fontWeight: "700" }}>{user.username}</h2>
            <p style={{ margin: "0 0 4px 0", color: "rgba(255, 255, 255, 0.7)", fontSize: "14px" }}>{user.email}</p>
            {user.created_at && (
      ...
      <div style={{
        ...
        <button
          ...
          style={{
            ...
            color: activeTab === "saved" ? "#3b82f6" : "rgba(255, 255, 255, 0.6)",
            fontSize: "16px",
            fontWeight: "600",
            cursor: "pointer",
            borderRadius: 0,
        ...
        <button
          ...
          style={{
            ...
            background: "transparent",
            border: "none",
            borderBottom: activeTab === "finished" ? "3px solid #3b82f6" : "3px solid transparent",
            color: activeTab === "finished" ? "#3b82f6" : "rgba(255, 255, 255, 0.6)",
            fontSize: "16px",
            fontWeight: "600",
            cursor: "pointer",
            borderRadius: 0,
      ...
      <div className="saved-trips-shell">
        <section className="saved-trips-sidebar">
          <div className="saved-trips-heading">
            <h1>{activeTab === "finished" ? "Finished Trips" : "Saved Trips"}</h1>
          ...
          {loading ? (
            ...
            <div className="saved-trip-empty">
              <h3>No {activeTab === "finished" ? "finished trips" : "saved itineraries"}</h3>
              <p>
                {activeTab === "finished"
                  ? "Mark active trips as finished to see them here."
                  : "Generate an itinerary while logged in and it will appear here."}
              </p>
            </div>
          ) : (
        ...
        <section className="saved-trip-detail">
          ...
          {!selectedTrip ? (
            <div className="saved-trip-empty large">
              <h2>Select a trip</h2>
              <p>Your trip details, day plan, edit tools, and actions will show here.</p>
            </div>
          ) : (
            <>
              <div className="saved-trip-detail-header">
                ...
                <div className="saved-trip-actions" style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                  {selectedTrip.can_edit !== false && (
                    <button
                      onClick={() => toggleFinishTrip(selectedTrip)}
                      className="saved-trip-secondary-btn"
                      style={{
                        background: selectedTrip.is_finished ? "#10b981" : "#4b5563",
                        color: "white"
                      }}
                    >
                      {selectedTrip.is_finished ? "Reopen Trip" : "Finish Trip"}
                    </button>
                  )}
                  {selectedTrip.can_edit !== false && (
                    <button
